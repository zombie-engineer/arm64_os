...
extern void __armv8_cpuctx_eret_to_proc(uint64_t pc, uint64_t sp, uint64_t spsr_el1);
...
void switch_to_initial_task(task_t *t)
{
  __current_cpuctx = t->cpuctx;
  uint64_t pc = (uint64_t)t->work;
  uint64_t sp = t->stack_base;

  // IRQ not masked
  uint64_t spsr_el1 = 0x344;
  __armv8_cpuctx_eret_to_proc(pc, sp, spsr_el1);
}
