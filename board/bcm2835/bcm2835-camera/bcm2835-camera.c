...
#include "../vchiq/mmal-common.h"
#include "../vchiq/mmal-encodings.h"
#include "../vchiq/mmal-vchiq.h"
#include "../vchiq/mmal-msg.h"
#include "../vchiq/mmal-parameters.h"
...
#define min_t(__t, __a, __b) (__t)((__a) < (__b) ? (__a) : (__b))
			   ...
			   unsigned int resolutions[][2], int num_resolutions)
{
	...
	uint32_t param_size = sizeof(cam_info);
	int i;
	...
	for (i = 0; i < min_t(unsigned int, cam_info.num_cameras, num_resolutions); i++) {
...
static int mmal_init(struct bm2835_mmal_dev *dev)
{
	...
	if (ret < 0) {
		v4l2_err(dev, "%s: vchiq mmal init failed %d\n",
			 __func__, ret);
    return ret;
	}
	...
	if (camera->outputs < CAM_PORT_COUNT) {
		v4l2_err(dev, "%s: too few camera outputs %d needed %d\n",
	...
	ret = set_camera_parameters(dev->instance,
				    ...
				    dev);
	if (ret < 0) {
		v4l2_err(dev, "%s: unable to set camera parameters: %d\n",
			 __func__, ret);
		goto unreg_camera;
	}
	...
	if (ret == 0) {
		...
		for (i = 0; i < param_size / sizeof(uint32_t); i++) {
...
unreg_vid_encoder:
	printf("Cleanup: Destroy video encoder\n");
	vchiq_mmal_component_finalise(
		dev->instance,
		dev->component[COMP_VIDEO_ENCODE]);

unreg_image_encoder:
	printf("Cleanup: Destroy image encoder\n");
	vchiq_mmal_component_finalise(
		dev->instance,
		dev->component[COMP_IMAGE_ENCODE]);

unreg_preview:
	printf("Cleanup: Destroy video render\n");
	vchiq_mmal_component_finalise(dev->instance,
				      dev->component[COMP_PREVIEW]);

unreg_camera:
	printf("Cleanup: Destroy camera\n");
	vchiq_mmal_component_finalise(dev->instance,
				      dev->component[COMP_CAMERA]);
...
int bcm2835_mmal_probe(struct bm2835_mmal_dev *dev)
{
	int ret;
//	struct vb2_queue *q;
	int camera;
	...
	num_cameras = get_num_cameras(instance, resolutions, MAX_BCM2835_CAMERAS);
