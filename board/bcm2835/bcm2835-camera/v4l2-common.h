...
#define v4l_printk(level, name, adapter, addr, fmt, arg...) \
	printf("%s %d-%04x: " fmt, name, i2c_adapter_id(adapter), addr , ## arg)
...
int v4l2_ctrl_query_fill(struct v4l2_queryctrl *qctrl,
			 int32_t min, int32_t max, int32_t step, int32_t def);
...
struct v4l2_subdev *v4l2_i2c_new_subdev(struct v4l2_device *v4l2_dev,
		...
		uint8_t addr, const unsigned short *probe_addrs);
...
#define VIDIOC_INT_RESET		_IOW ('d', 102, uint32_t)
 ...
 * and height fields in the driver specific must equal to that of uint32_t, i.e. four
 * bytes.
 *
...
#define v4l2_find_nearest_size(array, array_size, width_field, height_field, \
			       width, height)				\
	({								\
		BUILD_BUG_ON(sizeof((array)->width_field) != sizeof(uint32_t) || \
			     sizeof((array)->height_field) != sizeof(uint32_t)); \
...
__v4l2_find_nearest_size(const void *array, size_t array_size,
			 ...
			 size_t height_offset, int32_t width, int32_t height);
