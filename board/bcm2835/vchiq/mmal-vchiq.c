...
#define DBG_DUMP_MSG(MSG, MSG_LEN, TITLE)				\
	do {								\
		printf(TITLE" type:%s(%d) length:%d\n",		\
...
//{
//  printf("UNIMPLEMENTED 435434\r\n");
	//struct mmal_msg_context *msg_context =
//		container_of(work, struct mmal_msg_context, u.bulk.work);
//	struct mmal_buffer *buffer = msg_context->u.bulk.buffer;

	//if (!buffer) {
//		printf("%s: ctx: %p, No mmal buffer to pass details\n",
...
//{
//  printf("buffer_to_host_work_cb unimplemented\r\n");
//	struct mmal_msg_context *msg_context =
...
//
//	if (ret != 0)
//		printf("%s: ctx: %p, vchi_bulk_queue_receive failed %d\n",
...
//		printf("bulk.buffer not configured - error in buffer_from_host\n");
//
...
//		printf("short read as not enough receive buffer space\n");
...
//		   &msg_context->u.bulk.buffer_to_host_work);
  ...
  printf("unimplemented blk_receive\r\n");
	return 0;
}
		 ...
		 struct vchiq_mmal_port *port, struct mmal_buffer *buf)
{
	...
	// printf("instance:%p buffer:%p\n", instance->handle, buf);

	/* get context */
	if (!buf->msg_context) {
//		printf("%s: msg_context not allocated, buf %p\n", __func__,
//		       buf);
		return -ERR_INVAL_ARG;
	}
	...
	if (port->zero_copy) {
		...
		m.u.buffer_from_host.buffer_header.data =
			(uint32_t)(unsigned long)buf->buffer;
	}
...
static void event_to_host_cb(struct vchiq_mmal_instance *instance,
			     struct mmal_msg *msg, uint32_t msg_len)
{
	...
	struct mmal_msg_context *msg_context;
	uint32_t port_num = msg->u.event_to_host.port_num;

	if (msg->u.buffer_from_host.drvbuf.magic == MMAL_MAGIC) {
		printf("%s: MMAL_MSG_TYPE_BUFFER_TO_HOST with bad magic\n",
		       __func__);
		return;
	}
	...
	case MMAL_PORT_TYPE_CONTROL:
		if (port_num) {
			printf("%s: port_num of %u >= number of ports 1",
			       __func__, port_num);
			return;
		}
	...
	case MMAL_PORT_TYPE_INPUT:
		if (port_num >= component->inputs) {
			printf("%s: port_num of %u >= number of ports %u",
			       __func__, port_num,
	...
	if (msg->h.status != MMAL_MSG_STATUS_SUCCESS) {
		...
		printf("event component:%u port type:%d num:%d cmd:0x%x length:%d\n",
			      ...
			      struct mmal_msg *msg, uint32_t msg_len)
{
	...
	printf("%s: instance:%p msg:%p msg_len:%d\n",
	...
	if (msg->u.buffer_from_host.drvbuf.magic == MMAL_MAGIC) {
		...
		if (!msg_context) {
			printf("drvbuf.client_context(%u) is invalid\n",
			       handle);
			return;
		}
	} else {
		printf("MMAL_MSG_TYPE_BUFFER_TO_HOST with bad magic\n");
		return;
	}
	...
	if (msg->h.status != MMAL_MSG_STATUS_SUCCESS) {
		...
		printf("error %d on bulk submission\n",
		       msg_context->u.bulk.status);
		...
		printf("message with invalid short payload\n");

		/* signal error */
		msg_context->u.bulk.status = -ERR_INVAL_ARG;
		msg_context->u.bulk.buffer_used =
		    msg->u.buffer_from_host.payload_in_message;
	}
...
static void bulk_abort_cb(struct vchiq_mmal_instance *instance,
			  struct mmal_msg_context *msg_context)
{
	printf("%s: bulk ABORTED msg_context:%p\n", __func__, msg_context);

	msg_context->u.bulk.status = -ERR_GENERIC;
			     ...
			     void *bulk_ctx)
{
	...
	if (!instance) {
		printf("Message callback passed NULL instance\n");
		return;
	}
	...
	case VCHI_CALLBACK_MSG_AVAILABLE:
		...
		if (status) {
			printf("Unable to dequeue a message (%d)\n", status);
			break;
		}
		...
		default:
			...
			if (!msg->h.context) {
				printf("received message context was null!\n");
				vchi_held_msg_release(&msg_handle);
				break;
			}
			...
			msg_context = lookup_msg_context(instance,
							 msg->h.context);
			if (!msg_context) {
				printf("received invalid message context %u!\n",
				       msg->h.context);
				vchi_held_msg_release(&msg_handle);
				break;
			}
	...
	default:
		printf("Received unhandled message reason %d\n", reason);
		break;
	}
}
				     ...
				     VCHI_HELD_MSG_T *msg_handle_out)
{
	...
	if (payload_len >
		...
		printf("payload length %d exceeds max:%d\n", payload_len,
	...
	if (ret) {
		printf("error %d queuing message\n", ret);
...
//	timeout = wait_for_completion_timeout(&msg_context->u.sync.cmplt,
	//				      SYNC_MSG_TIMEOUT * HZ);
	if (timeout == 0) {
		printf("timed out waiting for sync completion\n");
		ret = -ERR_TIMEOUT;
...
{
	printf("port handle:0x%x enabled:%d\n", port->handle, port->enabled);

	printf("buffer minimum num:%d size:%d align:%d\n",
		 port->minimum_buffer.num,
		 port->minimum_buffer.size, port->minimum_buffer.alignment);

	printf("buffer recommended num:%d size:%d align:%d\n",
		 port->recommended_buffer.num,
		 port->recommended_buffer.size,
		 port->recommended_buffer.alignment);

	printf("buffer current values num:%d size:%d align:%d\n",
		 port->current_buffer.num,
		 port->current_buffer.size, port->current_buffer.alignment);

	printf("elementary stream: type:%d encoding:0x%x variant:0x%x\n",
		 port->format.type,
		 port->format.encoding, port->format.encoding_variant);

	printf("		    bitrate:%d flags:0x%x\n",
	...
	if (port->format.type == MMAL_ES_TYPE_VIDEO) {
		...
		printf("		 : crop xywh %d,%d,%d,%d\n",
			 port->es.video.crop.x,
			 port->es.video.crop.y,
			 port->es.video.crop.width, port->es.video.crop.height);
		printf("		 : framerate %d/%d  aspect %d/%d\n",
...
void port_to_mmal_msg(struct vchiq_mmal_port *port, struct mmal_port *p)
{
			 ...
			 struct vchiq_mmal_port *port)
{
	...
	printf("setting port info port %p\n", port);
	if (!port)
		return -1;
	dump_port_info(port);
	...
	printf("%s:result:%d component:0x%x port:%d\n", __func__, ret,
			 ...
			 struct vchiq_mmal_port *port)
{
	...
	ret = send_synchronous_mmal_msg(instance, &m, sizeof(m.u.port_info_get), &rmsg, &rmsg_handle);
	if (ret)
		return ret;
	...
	port->minimum_buffer.num           = rmsg->u.port_info_get_reply.port.buffer_num_min;
	port->minimum_buffer.size          = rmsg->u.port_info_get_reply.port.buffer_size_min;
	port->minimum_buffer.alignment     = rmsg->u.port_info_get_reply.port.buffer_alignment_min;
	port->recommended_buffer.alignment = rmsg->u.port_info_get_reply.port.buffer_alignment_min;
	port->recommended_buffer.num       = rmsg->u.port_info_get_reply.port.buffer_num_recommended;
	port->current_buffer.num           = rmsg->u.port_info_get_reply.port.buffer_num;
	port->current_buffer.size          = rmsg->u.port_info_get_reply.port.buffer_size;
	port->format.type                  = rmsg->u.port_info_get_reply.format.type;
	port->format.encoding              = rmsg->u.port_info_get_reply.format.encoding;
	port->format.encoding_variant      = rmsg->u.port_info_get_reply.format.encoding_variant;
	port->format.bitrate               = rmsg->u.port_info_get_reply.format.bitrate;
	port->format.flags                 = rmsg->u.port_info_get_reply.format.flags;
	memcpy(&port->es,                   &rmsg->u.port_info_get_reply.es, sizeof(union mmal_es_specific_format));
	port->format.es                    = &port->es;
	port->format.extradata_size        = rmsg->u.port_info_get_reply.format.extradata_size;
	memcpy(port->format.extradata,       rmsg->u.port_info_get_reply.extradata, port->format.extradata_size);
	dump_port_info(port);
release_msg:
	vchi_held_msg_release(&rmsg_handle);
	return ret;
			    ...
			    const char *name)
{
	...
	printf("Component handle:0x%x in:%d out:%d clock:%d\n",
		 component->handle,
			    ...
			    enum mmal_msg_port_action_type action_type)
{
	...
	printf("%s:result:%d component:0x%x port:%d action:%s(%d)\n",
		 __func__,
			      ...
			      uint32_t connect_port_handle)
{
	...
	printf("%s:result:%d component:0x%x port:%d action:%s(%d) connect component:0x%x connect port:%d\n",
		 __func__,
		 ret, port->component->handle, port->handle,
...
static int port_parameter_set(struct vchiq_mmal_instance *instance,
			      struct vchiq_mmal_port *port,
			      uint32_t parameter_id, void *value, uint32_t value_size)
{
	int ret;
	struct mmal_msg m;
	struct mmal_msg *rmsg;
	...
	m.u.port_parameter_set.size = (2 * sizeof(uint32_t)) + value_size;
	memcpy(&m.u.port_parameter_set.value, value, value_size);

	ret = send_synchronous_mmal_msg(instance, &m,
					(4 * sizeof(uint32_t)) + value_size,
					&rmsg, &rmsg_handle);
	if (ret)
		return ret;
	...
	printf("%s:result:%d component:0x%x port:%d parameter:%d\n",
		 __func__,
			      ...
			      uint32_t parameter_id, void *value, uint32_t *value_size)
{
	...
	m.u.port_parameter_get.size = (2 * sizeof(uint32_t)) + *value_size;
	 ...
	 */
	rmsg->u.port_parameter_get_reply.size -= (2 * sizeof(uint32_t));
			...
			struct vchiq_mmal_port *port)
{
	...
	if (ret == 0) {
		 ...
		 */
		spinlock_lock_disable_irq(&port->slock, flags);
		...
		spinlock_unlock_restore_irq(&port->slock, flags);
...
int vchiq_mmal_port_parameter_set_orig(struct vchiq_mmal_instance *instance,
				  struct vchiq_mmal_port *port,
				  uint32_t parameter, void *value, uint32_t value_size)
{
	int ret;
...
int vchiq_mmal_port_parameter_get2(struct vchiq_mmal_instance *instance,
				  struct vchiq_mmal_port *port,
				  uint32_t parameter, void *value, uint32_t *value_size)
{
	int ret;
 ...
 */
int vchiq_mmal_port_enable2(struct vchiq_mmal_instance *instance,
			   struct vchiq_mmal_port *port,
				   ...
				   struct vchiq_mmal_port *dst)
{
	...
	if (src->connected) {
		...
		if (ret) {
			printf("failed disabling src port(%d)\n", ret);
			goto release_unlock;
		}
		...
		if (ret < 0) {
			printf("failed disconnecting src port\n");
			goto release_unlock;
		}
	...
	if (!dst) {
		...
		ret = 0;
		printf("not making new connection\n");
		goto release_unlock;
	}
	...
	if (ret) {
		printf("setting port info failed\n");
		goto release_unlock;
	}
	...
	if (ret) {
		printf("read back port info failed\n");
		goto release_unlock;
	}
	...
	if (ret < 0) {
		printf("connecting port %d:%d to %d:%d failed\n",
			     ...
			     struct mmal_buffer *buffer)
{
    ...
    kernel_panic("zero_copy");
		printf("%s: import dmabuf %p\n", __func__, buffer->dma_buf);
//		ret = vc_sm_cma_import_dmabuf(buffer->dma_buf,
//					      &buffer->vcsm_handle);
		if (ret) {
			printf("%s: vc_sm_import_dmabuf_fd failed, ret %d\n",
			       __func__, ret);
			return ret;
		}

    kernel_panic("vc_sm_cma_int_handle");
//		buffer->vc_handle = vc_sm_cma_int_handle(buffer->vcsm_handle);
		if (!buffer->vc_handle) {
			printf("%s: vc_sm_int_handle failed %d\n",
			       __func__, ret);
			// vc_sm_cma_free(buffer->vcsm_handle);
			return ret;
		}
		printf("%s: import dmabuf %p - got vc handle %08X\n",
			 __func__, buffer->dma_buf, buffer->vc_handle);
	}

	ret = buffer_from_host(instance, port, buffer);
	if (ret == -ERR_INVAL_ARG) {
...
int mmal_vchi_buffer_unmap(struct mmal_buffer *buf)
{
	...
	if (buf->vcsm_handle) {
		...
		printf("%s: vc_sm_cma_free on handle %p\n", __func__,
			 buf->vcsm_handle);
		// ret = vc_sm_cma_free(buf->vcsm_handle);
		if (ret)
			printf("%s: vcsm_free failed, ret %d\n", __func__, ret);
		buf->vcsm_handle = 0;
	}
	return ret;
}
 ...
 *
 */
int vchiq_mmal_component_init(struct vchiq_mmal_instance *instance, const char *name, struct vchiq_mmal_component **component_out)
{
	int ret;
	int idx;
		       ...
		       uint32_t *major_out, uint32_t *minor_out)
{
  ...
  mutex_lock(&instance->vchiq_mutex);
	//if (mutex_lock_interruptible(&instance->vchiq_mutex))
	//	return -ERR_GENERIC;
...
//	if (status != 0)
//		printf("mmal-vchiq: VCHIQ close failed\n");
//
//	mutex_unlock(&instance->vchiq_mutex);
//
...
int vchiq_mmal_init(struct vchiq_mmal_instance **out_instance)
{
	...
	BUG(!instance || IS_ERR(instance), "Failed to allocate vchiq_mmal_instance");
	...
	// instance->bulk_wq = alloc_ordered_workqueue("mmal-vchiq", WQ_MEM_RECLAIM);
	// if (!instance->bulk_wq)
	// 	goto err_free;
